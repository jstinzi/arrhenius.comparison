...
```
Basically need to run week 1 and week 2 code for 6 days of environmental data and compile
#Week 1 modeling
```{r}
...
modelled[[i]] <- calculate_photo(data_phys = data_phys[[i]],
                            ...
                            varnames = list(GammaStar = "GammaStar",
                                            ...
                                            total_respiration = "total_respiration_plant",
                                            Ca = "Ca",
                                            Qin = "Qin",
                                            VPD = "VPD",
                                            Temp = "Temp",
...
for(i in 1:length(modelled2)){
  dailyC2[[i]] <- modelled2[[i]][[2]]
}
dailyC2 <- do.call("rbind", dailyC2)


summary(lm(dailyC$daily_C_plant ~ dailyC2$daily_C_plant))
BIC(lm(dailyC$daily_C_plant ~ dailyC2$daily_C_plant))
BIC(lm(dailyC$daily_C_plant ~ dailyC2$daily_C_plant-1))
plot(dailyC$daily_C_plant ~ dailyC2$daily_C_plant)
summary(lm(dailyC$daily_photosynthesis ~ dailyC2$daily_photosynthesis))
plot(dailyC$daily_photosynthesis ~ dailyC2$daily_photosynthesis)

comps <- data.frame(c(1:334))
comps$DailyC <- c(dailyC$daily_C_plant, dailyC2$daily_C_plant)
comps$Photosynthesis <- c(dailyC$daily_photosynthesis, dailyC2$daily_photosynthesis)
comps$Type <- c(rep("Medlyn", 167), rep("New", 167))
t.test(DailyC ~ Type, comps, paired = TRUE)
t.test(Photosynthesis ~ Type, comps, paired = TRUE)

boxplot((dailyC$daily_C_plant - dailyC2$daily_C_plant) * 365) #g/year
```
