...
vignette: >
 %\VignetteIndexEntry{Modeling_carbon_balance}
 %\VignetteEngine{knitr::rmarkdown}
 %\VignetteEncoding{UTF-8}
---

To run vignette, use find and replace searching for "#```", and replace
the phrase with the octothorpe removed.
...
pars_v <- read.csv(system.file("extdata", "fits_v_3.csv", 
                             package = "arrhenius.comparison"),
                 stringsAsFactors = FALSE)
pars_j <- read.csv(system.file("extdata", "fits_j_3.csv", 
                             package = "arrhenius.comparison"),
                 stringsAsFactors = FALSE)
...
data_p <- calculate_p(G25 = 42.75, #Bernacchi et al. 2001
                      ...
                      Km_Ea = 65.50828, #Bernacchi et al. 2001
                      Temp = data_env$Temp,
                      Vcmax25_m = pars_v$k25.medlyn[i],
                      Vcmax_dS_m = pars_v$delS.medlyn[i],
                      Vcmax_Hd_m = pars_v$Hd.medlyn[i],
                      Vcmax_Ea_m = pars_v$Ea.medlyn[i],
                      Vcmax25_n = pars_v$k25.new[i],
                      Vcmax_dS_n = pars_v$delS.new[i],
                      Vcmax_Hd_n = pars_v$Hd.new[i],
                      Vcmax_Ea_n = pars_v$Ea.new[i],
                      Jmax25_m = pars_j$k25.medlyn[i],
                      Jmax_dS_m = pars_j$delS.medlyn[i],
                      Jmax_Hd_m = pars_j$Hd.medlyn[i],
                      Jmax_Ea_m = pars_j$Ea.medlyn[i],
                      Jmax25_n = pars_j$k25.new[i],
                      Jmax_dS_n = pars_j$delS.new[i],
                      Jmax_Hd_n = pars_j$Hd.new[i],
                      Jmax_Ea_n = pars_j$Ea.new[i])
data_p <- data_p[, 1:6] #Removes temp column to preclude issues later
data_phys[[i]] <- cbind(data_r, data_p)
}
...
lm(week_1_carbon_Hdfit[week_1_carbon_Hdfit$ID == "Medlyn",]$daily_plant_photosynthesis ~ week_1_carbon_Hdfit[week_1_carbon_Hdfit$ID == "New",]$daily_plant_photosynthesis)
...
lm(week_1_carbon_Hdfit[week_1_carbon_Hdfit$ID == "Medlyn",]$daily_plant_AR ~ week_1_carbon_Hdfit[week_1_carbon_Hdfit$ID == "New",]$daily_plant_AR)
...
#Remove octothorpe below to run chunk
#```{r include = FALSE}
carbon <- rbind(week_1_carbon_Hdfit, week_12_carbon_Hdfit)
gasex <- rbind(week_1_ge, week_12_ge)
write.csv(week_1_carbon_Hdfit, "carbon_week1.csv")
write.csv(week_12_carbon_Hdfit, "carbon_week12.csv")
...
FigData <- read.csv(system.file("extdata", "FigModelingData2.csv", 
                             package = "arrhenius.comparison"),
                 stringsAsFactors = FALSE)
...
W1_HdVar_CGain <- ggplot(data = FigData, aes(x = week1_C_new, y = week1_C_med) +
  ggtitle(label = expression("e)"~"Low"~"R"~"With"~"Variable"~H[d])) +
  geom_point(size = 3, alpha = 0.7)+
  geom_abline(slope = 1, intercept = 0, size = 1, colour = "Black") +
